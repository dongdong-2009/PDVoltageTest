...
void __fastcall TFrmMain::btnStartClick(TObject *Sender)
{
	...
	else
	{
		...
		for(int i = 0;i < 5;i++)
		{
			...
			{
				Voltage_Test(SetVoltage[zVoltageSetting[i]->Hint.ToInt()]);
				break;
			}
		}
	}
...
void __fastcall TFrmMain::pl_Auto5VClick(TObject *Sender)
{
	...
	for(int i = 0;i<5;i++)
	{
		if(zAuto_Test[i]->Name == pl->Name)
		  zAuto_Test[i]->Caption = "V";
		else
		{
			zAuto_Test[i]->Caption = "";
			zRead_AD_Voltage[i]->Caption = "";
		}
	}
	if(btnStart->Caption == "Stop")
...
void TFrmMain::ReadRegSet()
{
	...
	try
	{
		...
		bool bl = reg->OpenKey("PD Voltage Test for RD", true);
...
void __fastcall TFrmMain::FormClose(TObject *Sender, TCloseAction &Action)
{
	...
	else MessageBoxA(NULL,"請按Stop鍵，按完後程序將自動關閉。","WARNING", MB_ICONEXCLAMATION);
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmMain::plSet5VClick(TObject *Sender)
{
	...
	pl_Auto5VClick(zAuto_Test[pl->Hint.ToInt()]);
}
//---------------------------------------------------------------------------
